...
final class ConsolidationPromptBuilder
{
    ...
    public function build(ConsolidationInput $input): string
    {
        ...
        $lines = [
            ...
            '',
            'Choose the durable target with target_type. A durable proposal may target a constraint:',
            '- constraint: consider when validated evidence describes a repeatable forbidden state that can be',
            '  deterministically rejected by PHPStan, PHPCS, PHP-CS-Fixer, a regression test, or CI.',
            '- Prefer the cheapest reliable owner; soft guidance stays correct when the state is not objectively detectable.',
            '- Do not propose a constraint merely because a finding exists, and do not propose one for a state that',
            '  needs prose interpretation or whole-program tracing to detect.',
            '- A constraint proposal supplies the constraint specification and remains a proposal: explicit review and',
            '  approval are still required, and nothing here activates a rule.',
            '',
